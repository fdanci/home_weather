...
class Source1:
    ...
    def today(location_id):
        ...
        url = "http://dataservice.accuweather.com/forecasts/v1/daily/1day/" + location_id + "" \
                                                                                            "?apikey=" + config(
            'ACC_WEATHER_KEY') + "&language=ro&details=true&metric=true"
    ...
    @staticmethod
    def forecast_12hours(location_id):
        """
        Read weather forecast for 12 hours.

        :return: The forecast in Json format.
        """
        url = "http://dataservice.accuweather.com/forecasts/v1/hourly/12hour/" + location_id + "" \
                                                                                               "?apikey=" + config(
            'ACC_WEATHER_KEY') + "&language=ro&details=true&metric=true"
